...
This is a PHP-based task management and email reminder system. Users can add tasks to a shared list and subscribe with their email to receive hourly reminders of pending tasks. The system uses text files for storage and sends HTML emails with verification and unsubscribe functionality.
...
- `run_cron.bat` (Windows batch file to run cron.php manually)  
...
- **`cron.php`** (Must handle sending task reminders via email)  
- **`run_cron.bat`** (Optional: Windows users can run this batch file manually to trigger cron.php)
...
- Task list must have `class="tasks-list"`.  
- Task item in that list must have `class="task-item"`  
- Task item must have a checkbox `<input type="checkbox" class="task-status" >` so user can mark it done.  
...
- The `BASE_URL` constant in `functions.php` should be updated to your accessible domain or IP and port for email links to work correctly.  
- Debug logs are available for troubleshooting:  
  - `task_add.log` records task addition attempts.  
  - `cron_log.txt` records cron job execution and email sending.  
- Windows users can use the `run_cron.bat` file to manually trigger the cron job if they cannot use the shell script.
